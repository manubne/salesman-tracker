...
class Api {
  ...
  static Future<List<Map<String, dynamic>>> customers({
    ...
    if (search != null && search.isNotEmpty) {
      q = q.or(
        'name.ilike.%$search%,company.ilike.%$search%,mobile.ilike.%$search%',
      );
    }
    if (verifiedOnly) {
      q = q.not('mobile_verified_at', 'is', null);
    }
  ...
  static Future<Map<String, dynamic>> customerOtp(
    String customerId, {
    String? code,
  }) async {
  ...
  static Future<Map<String, dynamic>> requestVisitOtp(
    String visitId,
    LocationFix fix,
  ) async {
    final res = await supa.functions.invoke(
  ...
  static Future<void> completeVisit(String visitId, {String? notes}) async {
    await supa
        .from('visits')
        .update({'status': 'completed', if (notes != null) 'notes': notes})
        .eq('id', visitId);
  }
  ...
  static Future<void> dayStart(LocationFix fix) async {
    await supa.rpc(
      ...
      params: {
        ...
        'p_mock_location': fix.mockLocation,
      },
    );
  }
  ...
  static Future<void> uploadOrderFile(
    String orderId,
    List<int> bytes,
    String kind,
  ) async {
    final path =
        '$orderId/${kind}_${DateTime.now().millisecondsSinceEpoch}.jpg';
    await supa.storage
        .from('order-files')
        .uploadBinary(path, Uint8List.fromList(bytes));
